<template lang="html">
  <div class="m-colorPicker" ref="colorPicker" @click="stopPro($event)">
    <!-- 颜色显示小方块 -->
    <div class="colorBtn"
    ...
    <div class="box" v-bind:class="{ open: openStatus }">
      ...
      <div class="bd" @click.stop>
        <h3 v-on:click="triggerHtml5Color">拾色器</h3>
        <span>当前色值：</span><input type="text"
          v-model="html5Color" class="currentStyle"
          v-on:change="updataValue(html5Color)">
        <h3>主题颜色</h3>
        <ul class="tColor">
          <li
            v-for="(color, i) of tColor"
            :key="i"
...
export default {
  ...
  watch: {
    openStatus (newVal, old) {

    },
    value (newVal) {
      this.html5Color = newVal
    }
  },
  data () {
    return {
  ...
  methods: {
    stopPro (evt) {
      evt.stopPropagation()
      let dom = document.getElementsByClassName('open')[0]
      if (dom) {
